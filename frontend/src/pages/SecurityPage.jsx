...
import { Lock, Shield, Eye, EyeOff, ArrowLeft, CheckCircle } from 'lucide-react';
...
export default function SecurityPage() {
  ...
  const strength = !passwordData.newPassword
    ? null
    : isStrong(passwordData.newPassword)
    ? 'strong'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('변경 실패: ' + (error.response?.data?.error || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fields = [
    { key: 'oldPassword',     label: '현재 비밀번호',     showKey: 'old',     icon: Lock },
    { key: 'newPassword',     label: '새 비밀번호',       showKey: 'new',     icon: Lock },
    { key: 'confirmPassword', label: '새 비밀번호 확인',  showKey: 'confirm', icon: Lock },
  ];
    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-100 dark:border-gray-700 p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">비밀번호 변경</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div key={key}>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={label}
                  className="w-full px-4 py-2.5 pr-11 border border-gray-300 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition"
                  required
                />
                <button
                  type="button"
              ...
              {key === 'newPassword' && strength && (
                <div className="mt-2">
                  ...
                  <p className={`text-xs font-medium ${strengthConfig[strength].text}`}>
                    비밀번호 강도: {strengthConfig[strength].label}
                    {strength === 'weak' && ' — 8자 이상, 대문자, 숫자 포함 권장'}
                  </p>
                </div>
              )}
              {/* 확인 비밀번호 일치 여부 */}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 bg-primary-600 text-white rounded-xl font-semibold hover:bg-primary-700 transition disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
